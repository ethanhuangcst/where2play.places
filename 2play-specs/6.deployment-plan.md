    ...
    → Cloudflare (grey until Let's Encrypt OK)
        → NPM :443
            → where2play-web:3000  (stack where2play)
                ...
                → product persistence TBD (see §6)
```

Thin app: **no** map vendor keys. Itinerary **engine** stays on places-agent ([ADR-008](../../workspace-specs/adr/ADR-008-itinerary-ownership.md)). Product LLM (`OPENAI_*`) is TBD until a where2play tech spec lands; **do not** copy what2eat’s “no OPENAI” rule blindly if this app keeps a local product loop.
...
| `DATABASE_URL` | TBD | Only if a persistence ADR lands |
| `OPENAI_*` | TBD | Product Quanzil — not map keys; not the agent’s keys |
| `SESSION_SECRET` | if cookies | Consumer session |

**Forbidden:** map vendor keys; `NEXT_PUBLIC_PLACES_AGENT_*`.
...
| Item | where2play |
| --- | --- |
| Engine | **TBD** — no ADR yet. Do not assume SQLite on a Portainer volume. Do **not** reuse Aliyun `places_agent`, `what2eat`, `kb_agent`, `mypoke_trade_prod`, `media_marketing`. |
| VPS volume | None unless a later spec adds uploads (release-bot ADR-001 if serving post-boot `public/`) |
