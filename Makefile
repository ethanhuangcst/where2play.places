...
up: ## Ensure Postgres on :5435 is reachable (use what2eat make up, or local Postgres)
	...
	elif [ -f ../what2eat/Makefile ] && command -v docker >/dev/null 2>&1; then \
		$(MAKE) -C ../what2eat up; \
	else \
		echo "Start Postgres on :$(PORT_PG) first (e.g. cd ../what2eat && make up)"; \
		exit 1; \
	fi

down: ## Note: shared Postgres is managed by what2eat
	@echo "Shared Postgres on :$(PORT_PG) is managed by what2eat (cd ../what2eat && make down if needed)."
