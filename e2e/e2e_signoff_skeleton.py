#!/usr/bin/env python3
"""Sign-off: 3-day Lisbon takeoff → skeleton thread. Servers must be up."""
...
EMAIL = "e2e.signoff.lx@where2play.place"
PASSWORD = "testpass123"
OUT_DIR = ROOT.parent / "specs" / "agent-specs" / "e2e-test-results"
SHOT = OUT_DIR / "signoff-lisbon-3d-ui.png"
UI_JSON = OUT_DIR / "_last-signoff-ui.json"
    ...
    with sync_playwright() as p:
        ...
        while time.time() < deadline:
            ...
            if err.count() and err.inner_text().strip():
                ...
                break
            if skel.count():
                notes["ui"]["skeleton_preview"] = skel.inner_text()[:2500]
                break
            page.wait_for_timeout(500)
        notes["ui"]["wait_s"] = round(time.time() - t0, 1)
        notes["ui"]["thread"] = page.locator('[data-testid="plan-nav-thread"]').inner_text()[:3500]
